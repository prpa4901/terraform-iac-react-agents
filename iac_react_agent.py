...
from agent_tools.terraform_file_manager import (
    read_tf_file_tool,
    list_tf_workspace_files_tool,
    generate_tf_file_tool,
)
...
def get_llm_response(ai_executor, message1, config, old_messages):
    ...
    response1 = ""
    for event in ai_executor.stream(
        {"messages": old_messages + [input_message]}, config, stream_mode="values"
    ):
        response1 = event
    return response1
...
def handle_assistant_response(response1, role):
    ...
    if role == "user":
        ...
        if isinstance(response1, dict) and response1.get("type") == "terraform_plan":
            ...
            "interface" in response1
            or "ip address" in response1
            or "Plan:" in response1
        ):
            st.markdown("### ☁️ Terraform Plan Summary")
            ...
            st.markdown(
                response1 if isinstance(response1, str) else json.dumps(response1)
            )
